...
自动化论文审稿与改进工具，基于 [Claude Code](https://claude.ai/claude-code) 构建。安装一次，即可在所有论文项目中使用。
...
- **安全兜底** — 每轮修改前自动 git checkpoint，失败自动回滚，修改按风险分级执行。
...
3. **Review** — 按目标期刊标准，从 7 个维度评分（技术严谨性、创新性、清晰度、实验设计、叙事逻辑、AI 味、可读性）
...
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| `review.venue` | 目标期刊/会议（如 `"NeurIPS 2026"`、`"Applied Energy"`、`"Operations Research"`、`"IEEE TSG"`），reviewer 据此调整评审标准。未设置时 agent 从论文内容自动推断 | 空 |
